...
static PulsadoresTask ( void *pvParameters )
{
    ...
    while(1)
    {
        if (xQueueReceive(cola1,&ui32Status,portMAX_DELAY)==pdTRUE){ // Cambiar portMAX_DELAY por 500 para apagado automatico 5 seg despues
            // de que la cola esta vacia
            // Decodifica lo recogido en ui32Status y enciende los LEDs correspondientes

            parametro.izq=ui32Status&LEFT_BUTTON; ///GPIOPinRead(GPIO_PORTF_BASE,LEFT_BUTTON);
            parametro.der=ui32Status&RIGHT_BUTTON;

            /*******************************************************************************/

            numdatos=TivaRPC_SendFrame(COMMAND_INTERRUPT,&parametro,sizeof(parametro));
...
int main(void)
{
    ...
    GPIOIntTypeSet(GPIO_PORTF_BASE, LEFT_BUTTON, GPIO_BOTH_EDGES);
    GPIOIntTypeSet(GPIO_PORTF_BASE, RIGHT_BUTTON, GPIO_BOTH_EDGES);
    ...
    if (xTaskCreate(PulsadoresTask, "Puls", TIVARPC_TASK_STACK, NULL, tskIDLE_PRIORITY+2, NULL) != pdTRUE)
    {
        while (1);
    }
    ...
    // Creo la cola
    cola1 = xQueueCreate(POSICIONES_COLA,sizeof(uint32_t));
    if( cola1 == NULL )
    {
        while(1);
    }
...
void GPIOFIntHandler(void){
    ...
    i32PinStatus=GPIOPinRead(GPIO_PORTF_BASE,ALL_BUTTONS);

    xQueueSendFromISR (cola1,&i32PinStatus,&higherPriorityTaskWoken);   //Escribe en la cola freeRTOS
